...
const stripeColors: Record<CalendarEvent["type"], string> = {
  general: "before:bg-slate-400",
  task: "before:bg-blue-500",
  meal: "before:bg-amber-500",
  reminder: "before:bg-violet-500",
}

export function EventPill({ event, usersById, onView, onEdit, compact }: EventPillProps) {
  ...
  return (
    ...
    >
      <div className={cn("flex items-center gap-2", compact && "gap-1")}> 
        ...
        {!compact && (
          <Badge variant="secondary" className="ml-auto h-5 text-[10px] px-1.5 py-0 capitalize">{event.type}</Badge>
        )}
      </div>
